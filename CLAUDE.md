...
## Поддомены для сторонних сервисов (Taplink и т.п.)

Добавлены CNAME-записи в DNS Vercel для домена westhim.kz:
- `link.westhim.kz` → `taplink.zone`
- `taplink.westhim.kz` → `taplink.zone`

Обе ведут на Taplink-страницу клиента (мини-сайт со ссылками: WhatsApp, сайт, 2ГИС, заявка).
Добавлять новые поддомены: `vercel dns add westhim.kz <имя> CNAME <target>`.

## Передача проекта west-him клиенту в Vercel — ЗАВЕРШЕНА (2026-06-30)

Проект `west-him` передан в личный (бесплатный) аккаунт клиента — команда **"West Him"** (Hobby), владелец `rahabek81-1376` / `raha.bek81@gmail.com`. Домены `westhim.kz` и `west-him.vercel.app` перенесены вместе с проектом, сайт работает (Status: Ready).

**Как был сделан перенос** (для справки, если понадобится повторить для другого проекта):
- В дашборде Transfer Project ищет получателя только среди команд, к которым у инициатора уже есть отношение (является участником) — поиск по email/username "чужого" аккаунта вслепую не находит ничего ("No results")
- Invite Members в команду клиента заблокирован на Hobby-плане (доступно только на Pro)
- Рабочий способ — официальный API-флоу переноса между несвязанными аккаунтами:
  1. Создать персональный токен на https://vercel.com/account/tokens со scope **Full Account**
  2. `POST https://api.vercel.com/v9/projects/{idOrName}/transfer-request` с `Authorization: Bearer <token>` → возвращает `code` (валиден 24 часа)
  3. Отправить получателю ссылку `https://vercel.com/claim-deployment?code=<code>` — он открывает её **под своим аккаунтом**, Vercel предлагает принять перенос
  4. Код одноразовый — после принятия использовать повторно нельзя
  5. Удалить временный токен после переноса (он имел полный доступ к аккаунту)

**Важно:** GitHub-репозиторий (`kereyevzh90-pixel/west-him`) при переносе НЕ передан — остаётся на аккаунте kereyevzh90-pixel. `git push` в `master` по-прежнему деплоит на тот же Vercel-проект (Git-связь сохраняется при transfer), но управлять DNS/доменом через `vercel dns add` из текущего окружения (kereyevzh90-pixel) больше нельзя — владелец сменился, доступ к Settings/Domains теперь только у клиента.
