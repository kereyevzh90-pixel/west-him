...
└── images/
    ├── IMG_0858.PNG                     ← hero главной (текущий, фон на весь экран)
...
- **Кнопки:**
  ...
  - `.cart-btn` — красная кнопка "Корзина" в шапке каталога
...
- Фото `IMG_0858.PNG` — фон на **весь** блок hero (`background-size: 80% auto; background-position: right center`)
...
1. **Header** — как везде, плюс кнопка "Корзина" с красным бейджем-счётчиком
2. **Hero** — серый фон `#f7f7f7`, только текст, бейдж + заголовок + описание (без кнопок прайса)
3. **Полоса цифр** `.props-strip` — 4 колонки
...
- Кнопки `.cat-item`: "Все товары", "Бытовая химия", "Автохимия"
- У каждой — счётчик `.cat-count` (заполняется из products.json)
- Активная категория: фон `#fff0f3`, цвет `#C8102E`
- На мобиле: горизонтальная прокрутка вместо колонки
...
- Если у товара есть `description` — текст отображается **поверх фото** внизу карточки: тёмный градиент + белый текст (`.prod-card-overlay` / `.prod-card-overlay-text`)
- Категория (красный uppercase), название, кнопка "В корзину"
- Кнопка `.prod-card-add` — красная; если товар в корзине — тёмная "В корзине (N)"
...
- "Оформить заказ в WhatsApp" — открывает `wa.me/77010706960` с текстом списка товаров
- "Очистить корзину"
...
- `updateCardButtons()` — обновить состояние кнопок "В корзину" на карточках
...
- `description`: короткий текст поверх фото (необязательно, например `"500 мл • Концентрат"`)
...
1. Header → Hero (серый фон) → Полоса цифр → Контент → Прайс-блок
...
2. В `index.html` изменить `url('images/...')` в CSS-правиле `.hero { background: ... }` и `background-size`/`background-position`
