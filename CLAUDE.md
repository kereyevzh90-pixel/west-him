...
Формат: JSON-массив объектов `{ id, name, category, image }`
...
- Модальное окно: поле названия, выбор категории, загрузка фото (превью 1/1)
...
- Первый вход: пароль (придумывает пользователь) + GitHub Token → localStorage
