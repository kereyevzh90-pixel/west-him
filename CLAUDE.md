...
## В процессе: передача проекта west-him клиенту в Vercel

Решили передать проект `west-him` целиком в личный (бесплатный) аккаунт клиента через Vercel **Transfer Project** — это бесплатно даёт ему полный контроль только над своим проектом (домен, DNS, поддомены), без необходимости платить за Vercel Pro ($20/мес) и без риска для остальных проектов в аккаунте kereyevzh90-pixel.

**Статус (на 2026-06-30):** email клиента получен — `raha.bek81@gmail.com`. Vercel CLI не поддерживает Transfer Project — это действие только через веб-панель Vercel, выполняется пользователем вручную.

**Следующий шаг (вручную, в Vercel Dashboard):**
1. В Vercel → Settings проекта west-him → Transfer → указать `raha.bek81@gmail.com`
2. Клиент принимает приглашение ("Project Transfer Request") в своём аккаунте (регистрируется на vercel.com на этот email, если ещё не зарегистрирован)
3. После передачи: GitHub-репозиторий (`kereyevzh90-pixel/west-him`) НЕ передаётся автоматически — это отдельная история; уточнить, нужно ли передавать и его
4. После того, как доступ передан клиенту — больше не получится управлять DNS/доменом через `vercel dns add` из этого окружения (аккаунт сменится) — проверить актуальность доступа перед следующими DNS-изменениями
