...
`.prod-card` — белый фон, рамка `1px solid #ebebeb`, `border-radius: 14px`, `cursor: pointer`:
- Фото сверху (1/1, object-fit: cover), при hover масштабируется 1.05×
- `description` товара — **скрыт** на карточке (`.prod-card-desc { display: none }`), показывается в попапе при клике
...
- **Попап описания** (`.desc-overlay` / `.desc-popup`): клик на карточку (кроме кнопки "В корзину") → маленькое окошко с названием и текстом описания. Закрывается кнопкой × или кликом за пределами.
...
- Модальное окно содержит поля:
  ...
  - **Описание** (description, необязательно) — показывается в попапе при клике на карточку в каталоге
